...
const ProductDetail = () => {
  ...
  const { hoaTangProducts, setHoaTangProducts } = useStore();
  ...
  const getProduct = () => {
    const foundProduct = HoaTangProducts.find(
  ...
  return product ? (
    <div className="container mx-auto px-4 md:px-16 lg:px-20 py-10">
          ...
          <div className="flex-1 flex flex-col gap-3 sm:gap-7">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex border border-gray-300 rounded">
                <button onClick={() => setQuantity(Math.max(0, quantity - 1))} className="px-2 py-1 border-r border-gray-300 hover:bg-gray-100">-</button>
                <input type="number" value={quantity} readOnly className="w-12 text-center focus:outline-none" />
                <button onClick={() => setQuantity(quantity + 1)} className="px-2 py-1 border-l border-gray-300 hover:bg-gray-100">+</button>
              </div>
            </div>
            <hr className="sm:w-4/5 w-full" />
            <button className="bg-green-500 text-white px-4 py-3 rounded text-sm active:bg-green-600 w-[50%]">Thêm vào giỏ hàng</button>
            <hr className="sm:w-4/5 w-full" />
            <div className="flex flex-col gap-2 text-sm text-gray-500 md:flex-row">
              <div className="flex items-center gap-2">
                <img src="https://shop.dalathasfarm.com/public/dalathasfarm/images/delivery.png" alt="" />
                <p>Giao hàng nhanh 2 giờ trong 5km</p>
              </div>
              <div className="flex items-center gap-2">
                <img src="https://shop.dalathasfarm.com/public/dalathasfarm/images/present-box.png" alt="" />
                <p>Miễn phí thiệp chúc mừng</p>
              </div>
              <div className="flex items-center gap-2">
                <img src="https://shop.dalathasfarm.com/public/dalathasfarm/images/eco-nature.png" alt="" />
      ...
      <RelatedProducts category="hoa-tang-hoa-dich-vu" products={hoaTangProducts} />
    </div>
  ) : (
